...
2. Apply the algorithm developed in (i) to compute $v_0(4, 4)$, which is the price of the Asian option at time zero.
3. Provide a formula for $\Delta_n(s, y)$, the number of shares of the underlying stock that must be held in the replicating portfolio at time $n$ if $S_n = s$ and $Y_n = y$.
